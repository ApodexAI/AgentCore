...
A ``TASK_WALL_TIME_MODE = "soft_research"`` workflow stops its agent loop
*early* so the downstream finalization stage (reporter or ``force_final_answer``)
can still complete inside whatever hard ceiling the platform imposes. Getting
that split right needs two numbers, not one:

``research_deadline_s``
    when the in-loop :class:`WallClockDeadlineObserver` stops research.

``hard_total_s``
    the externally enforced ceiling the whole task must fit inside — the
    ``MIROHARNESS_TASK_WALL_TIME_S`` env value (enforced in live ``serve`` by
    ``RenewableWallTimeLease``) or a legacy total-task ``wall_deadline_s``.
    ``0`` means "no known external ceiling".

The reserve subtracted from ``hard_total_s`` is only ever a *plan*:
``soft_wall_deadline_s`` floors research at half the wall so a generous reserve
can never starve it, which means on a short wall the reserve is smaller than
requested. :func:`remaining_phase_budget_s` is the matching safety net — the
finalization stage clamps its own ceiling to the time that is actually left
when it starts, so it fails open with a real answer instead of being killed
mid-call by the external ceiling.
"""
...
TASK_WALL_TIME_ENV = "AGENT_CORE_TASK_WALL_TIME_S"
LEGACY_TASK_WALL_TIME_ENVS = (
    "MIROHARNESS_TASK_WALL_TIME_S",
    "FRONTIER_AGENT_TASK_WALL_TIME_S",
)
RESEARCH_WALL_TIME_ENV = "RESEARCH_WALL_TIME"
REPORT_WALL_TIME_ENV = "REPORT_WALL_TIME"
...
def resolve_report_wall_time(
    agent_cfg: dict[str, Any],
    *,
    default: float,
    label_prefix: str,
    env_var: str = REPORT_WALL_TIME_ENV,
) -> float:
    """Resolve finalization reserve, with an optional deployment override.

    ``REPORT_WALL_TIME`` maps to the profile's internal
    ``wall_deadline_reserve_s`` field. The environment wins when it contains a
    valid non-negative duration; unset/invalid values preserve the profile.
    """
    profile_s = nonnegative_seconds(
        agent_cfg.get("wall_deadline_reserve_s"),
        default=default,
        label=f"{label_prefix} wall_deadline_reserve_s",
    )
    raw_env = os.environ.get(env_var)
    if raw_env is None or raw_env == "":
        return profile_s
    return nonnegative_seconds(raw_env, default=profile_s, label=env_var)
