...
A history that still holds an empty/truncated ``function.arguments`` value or an
orphan tool message makes *every* later request invalid on stricter
OpenAI-compatible gateways — including the finalization/report call that is
supposed to rescue the run. :func:`has_malformed_tool_protocol` detects that,
and :func:`build_recovery_context` flattens the useful visible text into a
single plain block that carries no tool protocol at all.

Sanitizers, nudge prefixes and entry labels are injected by the caller: the
per-workflow leaked-tool-call scrubbers and prompt wording differ, and this
module deliberately holds no workflow-specific text.
"""
...
from agent_core.runtime.loop.llm_client import estimate_text_tokens
