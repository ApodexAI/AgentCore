...
class SkillInjectionMiddleware(LLMMiddleware):
        ...
        try:
            ...
            for s in enabled_skills:
                ...
                if total_chars + len(entry) > self._SKILL_METADATA_MAX_CHARS:
                    ...
                    entries_list.append(f"  <!-- {omitted} more skill(s) omitted due to budget -->")
                    break
    ...
    def _resolve_loader(self) -> SkillLoader | None:
        ...
        if self._loader is not None:
            return self._loader
        try:
            from agent_core.components.skills import FileSystemSkillLoader
    ...
    def _role_can_load_skills(self, role_id: str) -> bool:
        ...
        if agent_reg is None:
            try:
                from agent_core.runtime.registries import services as registry
                from agent_core.runtime.registries.agents import (
                    AgentRegistry,
                )
    ...
    async def before_llm(self, ctx: LLMCallContext, messages: list[Message]) -> list[Message]:
