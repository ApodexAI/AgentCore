...
from agent_core.loop_types import BaseObserver, ToolResult, TurnContext
...
def _env_int_aliases(names: tuple[str, ...], default: int) -> int:
    for name in names:
...
def _caps() -> tuple[int, int, int]:
    ...
    return (
        _env_int_aliases(
            (
                "AGENT_CORE_REACT_STEP_RESULT_MAX_CHARS",
                "FRONTIER_AGENT_REACT_STEP_RESULT_MAX_CHARS",
                "MIROHARNESS_REACT_STEP_RESULT_MAX_CHARS",
            ),
            DEFAULT_RESULT_MAX_CHARS,
        ),
        _env_int_aliases(
            (
                "AGENT_CORE_REACT_STEP_THINKING_MAX_CHARS",
                "FRONTIER_AGENT_REACT_STEP_THINKING_MAX_CHARS",
                "MIROHARNESS_REACT_STEP_THINKING_MAX_CHARS",
            ),
            DEFAULT_THINKING_MAX_CHARS,
        ),
        _env_int_aliases(
            (
                "AGENT_CORE_REACT_STEP_ARGS_MAX_CHARS",
                "FRONTIER_AGENT_REACT_STEP_ARGS_MAX_CHARS",
                "MIROHARNESS_REACT_STEP_ARGS_MAX_CHARS",
            ),
            DEFAULT_ARGS_MAX_CHARS,
        ),
    )
