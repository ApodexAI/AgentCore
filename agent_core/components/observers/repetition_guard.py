...
* :class:`~agent_core.components.observers.duplicate_query_rollback.DuplicateQueryRollbackObserver`
  knows what a *search* is: it dedupes ``web_search`` requests over the
  whole loop and pops the turn instead of hinting.
* :class:`~agent_core.components.observers.text_repetition_guard.TextRepetitionGuard`
