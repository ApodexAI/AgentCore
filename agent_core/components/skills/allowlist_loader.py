...
class AllowlistSkillLoader:
    ...
    Implements :class:`agent_core.protocols.SkillLoader`
    ...
    def list_skills(self) -> list[Skill]:
        return [s for s in self.inner.list_skills() if s.skill_id in self.allowed_ids]
    ...
    def get_enabled_skills(self) -> list[Skill]:
        return [s for s in self.inner.get_enabled_skills() if s.skill_id in self.allowed_ids]
