...
Phase 7 (Skills three-layer split): renamed from
``plugins/skill_loader/loader.py:SkillLoader`` to live alongside other
components/ implementations. The old top-level ``skills/`` data
directory is now ``plugins/skills/`` (the default search path here).

References:
- DeerFlow: skills/loader.py (filesystem scan + frontmatter parsing)
- MiroFlow: skill/manager.py (frontmatter regex + SkillMeta)
"""
...
class FileSystemSkillLoader:
    ...
    Implements ``agent_core.protocols.SkillLoader``. Scans configured
    ...
    def __init__(
        ...
        self._skill_dirs = [Path(d) for d in (skill_dirs or [Path.cwd() / "plugins" / "skills"])]
