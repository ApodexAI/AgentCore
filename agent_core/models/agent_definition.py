...
class AgentDefinition(BaseModel):
    ...
    role_id: str = Field(
        ..., description="Unique identifier, e.g. 'researcher', 'critic', or any custom role"
    )
    display_name: str = Field(..., description="Human-readable name for UI display")
    system_prompt: str = Field(
        default="", description="System prompt sent to LLM when this agent acts"
    )
    allowed_tools: list[str] = Field(
        default_factory=list, description="Tool names this agent can use"
    )
    # Per-role LLM configuration (from miroharness_bin)
    model: str | None = Field(
        default=None, description="Override LLM model for this role (None = use default)"
    )
