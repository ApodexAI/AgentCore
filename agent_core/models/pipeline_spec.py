...
class ContextPolicy(BaseModel):
    ...
    filter_fn: str | None = Field(
        default=None,
        description=(
            "Dotted import path to (state: dict) -> dict. Takes precedence over include_fields."
        ),
    )
    inject_fields: list[str] = Field(
        default_factory=list,
        description=("Fields always injected by framework regardless of include_fields."),
    )
...
class CompressionConfig(BaseModel):
    ...
    max_field_tokens: dict[str, int] = Field(
        ...
        description=("Truncate state fields before prompt. E.g. {'evidence_cards': 8000}."),
    )
...
class SubAgentProfile(BaseModel):
    ...
    budget_fraction: float = Field(
        default=0.1,
        description=("Fraction of parent node's remaining budget allocated to this sub-agent"),
    )
...
class NodeDefinition(BaseModel):
    ...
    execution_policy: NodeExecutionPolicy | None = Field(
        default=None,
        description=(
            "Node-owned runtime policy such as tool gating. None = use workflow/framework defaults."
        ),
    )
    compression: CompressionConfig | None = Field(
        default=None,
        description=("Per-node compression config. None = framework defaults."),
    )
    ...
    prompt_template: str = Field(
        default="",
        description=("Jinja2 template. Used by generic executor when node_function is None."),
    )
...
class PhaseSpec(BaseModel):
    ...
    Kept for backward compatibility. Use PhaseSpec.to_node_definition()
    to convert, or PipelineSpec.resolved_nodes which auto-converts.
    """
    ...
    execution_policy: NodeExecutionPolicy | None = Field(
        default=None,
        description="Legacy bridge for node-owned tool/runtime policy.",
    )
...
class PipelineSpec(BaseModel):
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict[str, Any], description="Pipeline-level metadata"
    )
    hidden: bool = Field(
        default=False,
        description=(
