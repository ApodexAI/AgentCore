...
Anthropic's Messages API supports prompt caching but the feature is
**opt-in**: you must mark cacheable spans with ``cache_control:
{"type": "ephemeral"}``. Without it, every request bills the full
prompt as fresh input tokens (~$3/MTok on Claude Sonnet) instead of
the cache-hit rate (~$0.30/MTok).

This wrapper sits between the agent loop and the upstream Claude
client (Anthropic direct OR Claude via OpenRouter). It mutates the
leading ``system`` message ``{"content": str, "role": "system"}`` into
a single content-block form that carries ``cache_control: ephemeral``
so the system prompt span gets cached. Subsequent calls within the
cache TTL (5 min on Anthropic) that share the same system prompt land
on cache-read, billing at the discounted rate.

Wire by calling :func:`maybe_wrap_for_prompt_cache(client, provider,
model)` in the LLM construction site (``build_aux_llm`` /
``create_swarm_llm``). The helper short-circuits to ``client`` when
the model isn't Claude-family, so callers don't need to gate.

Scope notes:

- **Only the system message is cached today.** Tool definitions and
  long prior-turn tool messages are also cacheable but require deeper
  request-payload hooks; deferred.
- **No effect on non-Anthropic providers.** OpenAI / qwen / etc. just
  see the plain string content (both shapes are accepted on the wire).
- **Transparent under tool binding.** The native loop binds tools via
  ``_BoundLLM`` (``bind_tools``) *outside* the client, threading them
  through the per-call ``tools=`` kwarg. This adapter also exposes its
  own :meth:`AnthropicPromptCacheAdapter.bind_tools` that returns a
  fresh adapter carrying a default ``tools=`` payload — keeping the
  cache wrapper in front of the tool-augmented call. Caller-supplied
  ``tools`` override the bound default, matching native ``chat(...,
  tools=...)`` precedence.
"""
...
class AnthropicPromptCacheAdapter:
    """:class:`~agent_core.llm.LLMClient` wrapper that converts the
