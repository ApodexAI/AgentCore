...
# ── Reducer extraction ──────────────────────────────────────────────────
def extract_reducers(state_type: type) -> dict[str, Reducer]:
...
class MiniDAG:
    ...
    def set_reducers(self, reducers: dict[str, Reducer]) -> None:
    ...
    def validate(self) -> list[str]:
        ...
        for node_name in self.edges:
            if node_name in self.conditional_edges:
                errors.append(f"Node '{node_name}' has both static and conditional edges")
    ...
    async def astream(
        ...
        *,
        ...
        while current_node is not None and step < _MAX_STEPS:
            ...
            logger.debug(
                "miniDAG [%s] executing node '%s' (step %d)", thread_id, current_node, step
            )
            # Execute node
            try:
        ...
        if step >= _MAX_STEPS:
            logger.error(
                "miniDAG [%s] hit max steps (%d), forcing termination", thread_id, _MAX_STEPS
            )
