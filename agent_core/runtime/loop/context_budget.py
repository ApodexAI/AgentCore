...
def truncate_text_to_tokens(
    ...
    *,
    ...
    The budget bounds the *slice*: the search only ever compares
    ``estimator(chunk)`` against ``max_tokens - estimator(marker)``, never the
    concatenation it returns. That is exact for an additive estimator, but the
    module's own heuristic fallback is not additive — ``//4`` on the non-CJK
    count loses up to 3 characters' worth of remainder per operand — so
    ``estimator(result)`` may come in one token over ``max_tokens``. Callers
    sizing a request against a hard gateway limit should leave at least that
    much slack. An arbitrary injected estimator carries no bound at all.
    """
