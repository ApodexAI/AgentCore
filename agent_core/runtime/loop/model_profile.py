# pyright: reportMissingModuleSource=false, reportUnknownArgumentType=false, reportUnknownMemberType=false, reportUnknownVariableType=false
...
#
# The pattern table is product data configured with
# :func:`configure_model_registry`. It changes every time a host onboards a new
# endpoint, so keeping it out of core Python lets ops update one YAML file
# without changing the shared engine. The loader is cached for the process
# lifetime; restart or explicitly reset the cache to pick up edits.
...
def configure_model_registry(path: str | Path | None) -> None:
    """Set the host-owned model registry used for thinking-format inference.

    AgentCore deliberately ships no product endpoint registry. Hosts configure
    their packaged YAML path once from a compatibility facade; passing None
    disables registry inference and falls back to the caller's default.
    """
...
def _load_thinking_format_patterns() -> (
    ...
    """
    if _registry_path is None or not _registry_path.is_file():
        logger.debug(
...
class DefaultThinkingParser:
    ...
    - ``"reasoning_content"`` (DeepSeek-V4 API, gpt-5/o-series via standard
      ...
      :class:`~agent_core.llm.LLMResponse` surfaces it directly via
