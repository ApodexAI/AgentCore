...
from agent_core.runtime.loop.compact_llm import LLMSummaryCompactor, SummaryPromptBuilder
from agent_core.runtime.loop.summary_prompt import compaction_prompt
...
def compaction_trigger_tokens(max_len: int, ratio: float = DEFAULT_TRIGGER_RATIO) -> int:
    ...
    while every construction site shares validation and rounding semantics.
    """
...
class TieredCompactor:
    ...
    def _spill_changed_tool_results(
        ...
        if self._spill is None:
            return []
        id_to_name = tool_names_by_call_id(source)
        refs: list[str] = []
