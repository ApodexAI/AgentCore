# pyright: reportMissingTypeArgument=false, reportUnknownArgumentType=false, reportUnknownMemberType=false, reportUnknownParameterType=false, reportUnknownVariableType=false, reportUnusedFunction=false
...
class DefaultToolCallParser:
    ...
    def __init__(self, *, keep_unknown_native_companions: bool = False) -> None:
        """Configure how a mixed native batch handles unknown tool names.

        Unknown-only batches are always retained so the executor can return a
        corrective error. In a mixed batch, hosts may either discard unknown
        companions (the default) or retain every unambiguous native call.
        """
        self.keep_unknown_native_companions = keep_unknown_native_companions
    ...
    def parse(self, response: Any, tool_names: set[str]) -> list[dict]:
        ...
        if native:
            ...
            if known_calls and not self.keep_unknown_native_companions:
