...
class AgentRegistry:
    ...
    def get(self, role_id: str) -> AgentDefinition:
        ...
        if role_id not in self._agents:
            raise KeyError(
                f"Agent role '{role_id}' not registered. Available: {list(self._agents.keys())}"
            )
    ...
    def clone(self) -> AgentRegistry:
        """Return an independent registry with deep-copied definitions."""
        cloned = AgentRegistry()
        cloned._agents = {
            role_id: definition.model_copy(deep=True)
            for role_id, definition in self._agents.items()
        }
        return cloned
