...
from agent_core.messages import Message, assistant_msg, text_of, user_msg
    ...
    def replay_budget(self) -> int:
        ...
        return max(
            ...
            min(
                ...
                context_window - max(int(self.max_completion_tokens), 0) - self.execution_reserve,
            ),
        )
...
class SessionHistoryCompactor:
    ...
    def _clone(turns: Iterable[SessionTurn]) -> list[SessionTurn]:
        return [
            {
                **turn,
                "messages": [message.copy() for message in turn.get("messages") or []],
            }
            for turn in turns
        ]
    ...
    def _remove_tool_results(turn: SessionTurn) -> bool:
        ...
        filtered = [message for message in messages if message.get("role") != "tool"]
    ...
    async def _summarize(self, turns: Iterable[SessionTurn]) -> str:
        ...
        summary = "\n\n".join(
            ...
            if message.get("role") == "user" and text_of(message.get("content")).strip()
        )
