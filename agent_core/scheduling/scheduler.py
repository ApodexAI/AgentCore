...
from agent_core.runtime.dag.graph_builder import resolve_state_type
...
class ProcessManager(Protocol):
    async def get_task(self, task_id: TaskId) -> SchedulerTask: ...

    async def update_status(self, task_id: TaskId, status: TaskStatus) -> Any: ...
...
def resolve_wall_time_s(
    ...
    from agent_core.scheduling.workflow_defaults import get_workflow_default
    ...
    env_name = next(
        (name for name in _WALL_TIME_ENVS if os.environ.get(name, "").strip()),
        None,
    )
...
class Scheduler:
    ...
    def _get_graph(self, pipeline_id: str | None = None) -> _GraphType:
        ...
        # fan-in reducers). A spec that declares nothing now falls back to
        # AgentCore's ``BaseTaskState`` — the universal fields plus their
        # fan-in reducers — rather than the bare ``dict`` this used to get.
