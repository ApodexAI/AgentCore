...
A topology factory turns a name + options + model tiers into a
``PipelineSpec``. Keeping the registry here lets the API / router layer
dispatch ``build("solo", ...)`` without importing workflow packages
directly. Workflows register their factories via ``WorkflowContext`` at
bootstrap time.
"""
...
class TopologyRegistry:
    ...
    def build(
        ...
        try:
            ...
            raise KeyError(
                f"No topology factory registered for {name!r}. Did the owning workflow register it?"
            ) from exc
