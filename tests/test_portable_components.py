...
from agent_core.components.middleware.llm.summarization import SummarizationMiddleware
...
@pytest.mark.asyncio
async def test_summarization_accepts_product_addendum_provider() -> None:
    middleware = SummarizationMiddleware(
        threshold=1,
        keep_recent=1,
        summary_addendum=lambda: "[Structured findings]\nportable evidence",
    )
    messages = [user_msg("old " * 100), user_msg("middle " * 100), user_msg("recent")]
    compacted = await middleware.before_llm(LLMCallContext(), messages)
    assert "portable evidence" in text_of(compacted[0]["content"])
