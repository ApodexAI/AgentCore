...
from agent_core.runtime.session_history import (
...
def test_render_replays_turns_and_tool_results_in_order() -> None:
    ...
    assert prompt.index("Official page: released 2026-04-03") < prompt.index("It was April 3.")
    assert prompt.index("It was April 3.") < prompt.index("What source confirmed that?")
...
def test_compactor_drops_old_results_before_recent_turns() -> None:
    ...
    assert all(message["role"] != "tool" for message in result.turns[0]["messages"])
    assert any(message["role"] == "tool" for message in result.turns[-1]["messages"])
...
def test_compactor_summarizes_old_turns_and_keeps_last_five() -> None:
    ...
    assert result.turns[0]["summary"].endswith("old decisions and evidence rollup")
